...
simulate the creation of \ahmad{``rumors''} at the nodes: we flip a biased coin for each
node in the graph, where the bias depends on the out-degree of the node. We
assume a partition of the nodes into classes based on their out-degrees, \ahmad{where for the nodes in the same class, we assign the same head probability for their biased coins, as show in in Table~\ref{tab:bias}.} In Table~\ref{table:datasets}, for each
dataset we report the size of the classes and the expected number of flipped
coins with outcome head at each time (\ahmad{rightmost} column).
\begin{table}[ht]
	\begin{tabular}{lcl}
		\toprule
...
the ``diffusion'' phase we simulate the spreading of the \ahmad{``rumor''} originating
...
we generated an \ahmad{item/rumor} $(t,S)$.
